...
import { deleteCompany, getAllCompany } from '../../Lib/CompanyApi'; // Ensure this function accepts pagination
...
const columns = [
    ...
    { id: 'serviceTypeName', label: 'Vehicle Type', minWidth: 120, align: 'center' },
...
const CompanyManagementTabel = ({ handleOpenCompanyEdit }) => {

    const classes = useStyles();
    const { companyData, setCompanyData, page, setPage, rowsPerPage, setRowsPerPage, searchQuery, setSearchQuery } = useCompanySection()
    ...
    const fetchAllCompany = async (query = searchQuery) => {
        const response = await getAllCompany({ search: query, page, limit: rowsPerPage });
    ...
    const handleDelete = async (row) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let response = await deleteCompany(row?._id);
                if (response?.type === "success") {
                    fetchAllCompany()
                    Alert('Success', 'Your selected company was deleted successfully.', 'success');
                } else {
                        ...
                        {companyData.map((row) => {
                            ...
                            return (
                                <TableRow
                                    ...
                                    key={row._id}
                                    style={{
                                        backgroundColor: isDeleted ? '#f8d7da' : 'inherit', // Light red for deleted companys
                                    }}
                                >
                                    <TableCell className="DashboardTabelCell">{row.name}</TableCell>
                                    <TableCell className="DashboardTabelCell">{row.companyName}</TableCell>
...
export default CompanyManagementTabel;
