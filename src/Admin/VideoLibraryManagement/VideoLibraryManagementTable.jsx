...
import { deleteVideoLibrary, getAllVideoLibrary } from '../../Lib/VideoLibrarysApi'; // Ensure this function accepts pagination
...
const columns = [
    { id: 'title', label: 'Title', minWidth: 180, align: 'right' },
...
const VideoLibraryManagementTabel = ({ handleOpenVideoLibraryEdit }) => {

    const classes = useStyles();
    const { videoLibraryData, setVideoLibraryData, page, setPage, rowsPerPage, setRowsPerPage, searchQuery, setSearchQuery } = useVideoLibrarySection()
    ...
    const fetchAllVideoLibrary = async (query = searchQuery) => {
        const response = await getAllVideoLibrary({ search: query, page, limit: rowsPerPage });
        setVideoLibraryData(response?.videos || []);
        setTotalData(response?.totalVideos || 0);
    };

    const handleEdit = (row) => {
        setEditData(row);
        handleOpenVideoLibraryEdit(row);
    };
    ...
    const handleDelete = async (row) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let response = await deleteVideoLibrary(row?._id);
                if (response?.type === "success") {
                    fetchAllVideoLibrary()
                    Alert('Success', 'Your selected video-Library was deleted successfully.', 'success');
                }
            }
        })
                        ...
                        {videoLibraryData.map((row) => {
                            ...
                            return (
                                <TableRow
                                    ...
                                    key={row._id}
                                    style={{
                                        backgroundColor: isDeleted ? '#f8d7da' : 'inherit', // Light red for deleted videoLibrarys
                                    }}
                                >
                                    <TableCell className="DashboardTabelCell">{row.title}</TableCell>
                                    <TableCell className="DashboardTabelCell">{row.link}</TableCell>
                                    <TableCell
...
export default VideoLibraryManagementTabel;
