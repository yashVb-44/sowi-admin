...
import { deleteVendor, getAllVendor } from '../../Lib/VendorApi'; // Ensure this function accepts pagination
...
import VerifiedVendorIcon from '@mui/icons-material/VerifiedUser';
import HighlightOffIcon from '@mui/icons-material/HighlightOff';
import { useVendorSection } from '../../Context/VendorDetailsContext';

const columns = [
    { id: 'profileImage', label: 'Profile Image', minWidth: 50, align: 'center' },
    { id: 'name', label: 'Vendor Name', minWidth: 100 },
    { id: 'mobileNo', label: 'Mobile Number', minWidth: 80, align: 'left' },
    { id: 'serviceTypeName', label: 'Vehicle Type', minWidth: 80, align: 'left' },
    { id: 'gender', label: 'Gender', minWidth: 50, align: 'left' },
    { id: 'createdAt', label: 'Joining Date', minWidth: 80, align: 'left' },
    ...
    // { id: 'isApproved', label: 'Approved', minWidth: 50, align: 'center' },
...
const VendorManagementTabel = ({ handleOpenVendorEdit }) => {

    const classes = useStyles();
    const { vendorData, setVendorData, page, setPage, rowsPerPage, setRowsPerPage, searchQuery, setSearchQuery } = useVendorSection()
    ...
    const fetchAllVendor = async (query = searchQuery) => {
        const response = await getAllVendor({ search: query, page, limit: rowsPerPage });
        setVendorData(response?.vendors || []);
        setTotalData(response?.totalVendors || 0);
    };

    const handleEdit = (row) => {
        setEditData(row);
        handleOpenVendorEdit(row);
    };
    ...
    const handleDelete = async (row) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let response = await deleteVendor(row?._id);
                if (response?.type === "success") {
                    fetchAllVendor()
                    Alert('Success', 'Your selected vendor was deleted successfully.', 'success');
                }
            }
        })
                        ...
                        {vendorData.map((row) => {
                            ...
                            return (
                                <TableRow
                                    ...
                                    key={row._id}
                                    style={{
                                        backgroundColor: isDeleted ? '#f8d7da' : 'inherit', // Light red for deleted vendors
                                    }}
                                >
                                    ...
                                    >
                                        {row.isVerified ? (
                                            <VerifiedVendorIcon className="status-icon-verified" />
                                        ) : (
                        ...
                        {vendorData.map((row) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* <TableCell
                                        className={`DashboardTabelCell ${row.isApproved ? 'status-verified' : 'status-unverified'}`}
                                    >
                                        {row.isApproved ? (
                                            <VerifiedVendorIcon className="status-icon-verified" />
                                        ) : (
...
export default VendorManagementTabel;
